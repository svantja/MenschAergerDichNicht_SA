  ...
  override def action(): Try[_] = {
    ...
    if (c.gameState == NONE || c.gameState == PREPARE) {
      if (c.players.length < 4) {
          ...
          if (c.players.length != player.playerId) {
            println("Players length: " + c.players.length)
            println("PlayerId - 1: ", player.playerId - 1)
            player.playerId = c.players.length
...
case class ChooseToken(tokenId: Int, c: Controller) extends Command {
  ...
  override def action(): Try[_] = {
    println("Current Player: " + c.players.getCurrentPlayer)
    println(t)
    t match{
      case Some(token) => {
